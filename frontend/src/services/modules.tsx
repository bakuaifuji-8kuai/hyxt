...
export const MODULES: ModuleConfig[] = [
  // ===== 会员数字化 =====
  {
    ...
    doc: {
      overview: '会员等级体系是会员运营的核心框架，通过设置多级会员身份（普通、银卡、金卡、钻石等），为不同价值的会员提供差异化权益和服务体验。系统支持基于积分、消费金额或成长值的多种升级条件，配合保级规则、降级机制和等级有效期，构建完整的会员成长激励闭环。会员等级与会员权益、积分倍率、停车优惠、专属客服等模块深度联动，直接影响会员在各场景下的权益享受，是精细化运营和提升会员忠诚度的关键基础设施。',
      features: ['支持新增、编辑、删除会员等级，灵活构建多级会员体系', '提供三种升级条件类型：积分累计、消费金额、成长值，满足不同运营策略', '配置保级要求和降级规则（不降级/自动降级/手动降级），维持会员活跃度', '设置等级有效期（天），支持周期性等级评定和刷新', '配置等级专属折扣率（0-1），消费时自动享受对应折扣', '支持升级礼包配置，会员升级时自动发放券、积分等奖励', '等级图标和视觉样式配置，提升会员身份荣誉感', '等级启用/禁用控制，支持灵活调整等级体系结构', '与会员权益模块联动，按等级配置差异化权益组合', '与积分模块联动，支持等级积分倍率设置', '与停车权益模块联动，按等级配置免费停车时长', '支持等级排序和显示优先级配置'],
      tips: ['等级编码为系统唯一标识，创建后不建议修改，避免关联数据错乱', '折扣率取值范围为0-1，例如0.9表示9折优惠，设置前请测试计算逻辑', '保级要求建议采用自然语言描述，如"年消费满1000元"，便于会员理解', '升级条件值建议逐级递增，形成合理的成长阶梯，避免跨度过大或过小', '启用新等级前请检查权益配置是否完整，避免会员升级后无法享受对应权益']
    }
  },
  {
  ...
  // ===== 积分中心 =====
  {
    ...
    doc: {
      overview: '积分规则是积分体系的核心配置模块，定义会员在各种业务场景下获取积分的方式和数量标准。通过配置消费送积分、签到奖励、生日礼遇等多种积分获取规则，构建完整的积分激励体系，引导会员行为，提升会员活跃度和忠诚度。积分规则与会员档案、积分流水、会员等级等模块深度联动，积分获取后实时计入会员账户，并可用于后续的积分商城兑换、抵现消费等场景，是会员运营闭环中不可或缺的重要环节。',
      features: ['支持新增、编辑、删除积分规则，灵活配置多种积分获取场景', '提供多种规则类型：消费送积分、签到积分、生日积分等，覆盖核心运营场景', '配置积分奖励数值，支持固定值或按比例计算等多种方式', '设置规则条件说明，清晰描述积分获取的前置条件和适用范围', '支持规则启用/禁用，灵活控制积分发放策略', '消费送积分支持按消费金额比例计算，可设置最低起送门槛', '签到积分支持连续签到递增奖励，培养会员登录习惯', '生日积分支持生日当月/当日自动发放，增强会员关怀', '与积分流水模块联动，每笔积分发放自动生成流水记录', '与会员等级联动，支持等级积分倍率加成配置', '支持规则有效期设置，临时性活动规则可自动失效', '提供规则排序和优先级配置，避免多规则叠加冲突'],
      tips: ['配置消费积分规则时注意设置合理的积分比例，避免积分通胀', '签到建议设置连续签到阶梯奖励，如连续7天额外奖励，提升粘性', '生日积分建议提前测试发放时机，确保在会员生日当天准时到账', '修改积分规则后，建议同步检查积分商城商品定价是否合理', '积分规则启用前请进行小范围测试，验证计算逻辑正确性']
    }
  },
  {
